...
function OplataItalianPage() {
  ...
  // состояние userExists не используется, но оставлено для возможного расширения
  const courseImage = '/media/оплата курса по итальянскому.png';
  ...
  useEffect(() => {
    ...
    // Если язык не итальянский, но мы на странице итальянского - перенаправляем
    if (savedLanguage && savedLanguage !== 'italian') {
      if (savedLanguage === 'english') navigate('/oplata-english');
      else if (savedLanguage === 'german') navigate('/oplata-german');
      else if (savedLanguage === 'spanish') navigate('/oplata-spanish');
      else if (savedLanguage === 'chinese') navigate('/oplata-chinese');
      else if (savedLanguage === 'french') navigate('/oplata-french');
      else if (savedLanguage === 'japanese') navigate('/oplata-japanese');
      else if (savedLanguage === 'korean') navigate('/oplata-korean');
    }
  }, [navigate]);
  ...
  const checkLoginExists = async (login) => {
    try {
      ...
      // setUserExists(checkResult.loginExists); // состояние не используется
  ...
  const handleBuyClick = async (e) => {
    ...
    if (isValid) {
      try {
        ...
        // Создаем платеж
        const language = localStorage.getItem('courseLanguage') || 'italian';
...
export default OplataItalianPage;