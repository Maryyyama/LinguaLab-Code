...
function SpanishTeacherMethodicalWorkPage() {
  const navigate = useNavigate();
  const [loading, setLoading] = useState(true);

  // Данные для таблицы показателей (для испанского учителя)
  const metricsData = [
    { indicator: 'Эффективность методик', value: '92%', dynamics: '▲ +2', norm: '85%', comment: 'Выше среднего по школе' },
    { indicator: 'Усвоение материала', value: '4.8/5', dynamics: '▲ +0.1', norm: '4.5', comment: 'Хороший результат в группе' },
    { indicator: 'Инновационность подходов', value: '★★★★☆', dynamics: '★ +0', norm: '★★★', comment: 'Использует современные методики' },
    { indicator: 'Подготовка материалов', value: '9.0/10', dynamics: '▲ +0.3', norm: '8.5', comment: 'Отличная подготовка по испанскому' },
    { indicator: 'Вовлеченность учащихся', value: '94%', dynamics: '▲ +4%', norm: '90%', comment: 'Высокая активность на занятиях' },
    { indicator: 'Методическая дисциплина', value: '9/10', dynamics: '▲ +0', norm: '9.0', comment: 'Хорошее ведение документации' }
  ];

  // Данные для текущего модуля (испанский язык)
  const currentModule = {
    title: 'Модуль: 4, тип: Hola / Приветствие',
    deadline: '15-29 июня',
  ...
  // Статусы для плана преподавателя (для испанского учителя)
  const [teacherPlan, setTeacherPlan] = useState([
    { id: 1, task: 'Разработать 20 карточек с лексикой', status: 'done' },
  ...
  // Материалы и ресурсы (для испанского языка)
  const resources = [
    '[PDF] Словарь по теме «Приветствие и знакомство»',
    '[DOCX] Грамматический разбор времен в испанском языке',
    '[MP3] Аудио для упражнения «Введение себя»',
    '[YouTube] Видео разбор ошибок учащихся по испанскому языку (доступ только преподавателям)'
  ];
  ...
  useEffect(() => {
    ...
    const savedPlan = localStorage.getItem('spanishTeacherMethodicalPlan');
    if (savedPlan) {
  ...
  const changeStatus = (taskId, newStatus) => {
    ...
    localStorage.setItem('spanishTeacherMethodicalPlan', JSON.stringify(updatedPlan));
    setOpenDropdown(null);
  };

  // Навигация по страницам
  const goToPersonalInfo = () => {
    navigate('/teacher/spanish');
  };
          ...
          <section className="progress">
            ...
            <div className="progress__modules">
              ...
              <div className="achievements">
                ...
                <div className="achievements__club">Прогресс выполнения: 70%</div>
                <ul className="module__tasks">
...
export default SpanishTeacherMethodicalWorkPage;