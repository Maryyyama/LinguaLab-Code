...
function OplataChinesePage() {
  ...
  const [coursePrice, setCoursePrice] = useState(() => {
    const saved = localStorage.getItem('coursePrice');
    return saved ? `${saved} Р` : '6520 Р';
  });
  const [courseId, setCourseId] = useState(() => localStorage.getItem('currentCourseId') || '');
  const [courseLevel, setCourseLevel] = useState(() => localStorage.getItem('courseLevel') || 'beginner');
  const [courseGroup, setCourseGroup] = useState(() => localStorage.getItem('courseGroup') || 'Standard Group');
  const [loading, setLoading] = useState(false);
  const courseImage = '/media/оплата курса по китайскому.png';
  ...
  useEffect(() => {
    ...
    console.log('=== OplataChinesePage загрузка ===');
    ...
    // Если язык не китайский, но мы на странице китайского - перенаправляем
    if (savedLanguage && savedLanguage !== 'chinese') {
      if (savedLanguage === 'german') navigate('/oplata-german');
      else if (savedLanguage === 'spanish') navigate('/oplata-spanish');
      else if (savedLanguage === 'english') navigate('/oplata-english');
    }
  }, [navigate]);
  ...
  const handleBuyClick = async (e) => {
    ...
    if (isValid) {
      try {
        ...
        // Создаем платеж
        const language = localStorage.getItem('courseLanguage') || 'chinese';
...
export default OplataChinesePage;