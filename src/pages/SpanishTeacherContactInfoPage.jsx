...
function SpanishTeacherContactInfoPage() {
  const navigate = useNavigate();
  const [loading, setLoading] = useState(true);
  const [isEditing, setIsEditing] = useState(false);
  const [contactData, setContactData] = useState({
    phone: '+34 912 345 678',
    email: 'juanperez@lingualab.es',
    city: 'Мадрид',
    address: 'улица Гран-Виа, д. 45'
  });
  ...
  // Форматирование испанского телефона при вводе
  const formatSpanishPhoneNumber = (value) => {
  ...
  useEffect(() => {
    ...
    const savedContacts = localStorage.getItem('spanishTeacherContacts');
    if (savedContacts) {
  ...
  const handleChange = (e) => {
    ...
    if (name === 'phone') {
      newValue = formatSpanishPhoneNumber(value);
    }
  ...
  const validateForm = () => {
    ...
    if (!phoneRegex.test(contactData.phone)) {
      newErrors.phone = 'Введите корректный испанский номер в формате +34 XXX XXX XXX';
    }
  ...
  const handleSave = (e) => {
    ...
    localStorage.setItem('spanishTeacherContacts', JSON.stringify(contactData));
    setOriginalData(contactData);
    setIsEditing(false);
    setSaveSuccess(true);
...
export default SpanishTeacherContactInfoPage;