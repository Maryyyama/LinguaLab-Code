...
function OplataFrenchPage() {
  ...
  const courseImage = '/media/оплата курса по французскому.png';
  ...
  useEffect(() => {
    ...
    // Если язык не французский, но мы на странице французского - перенаправляем
    if (savedLanguage && savedLanguage !== 'french') {
      ...
      else if (savedLanguage === 'italian') navigate('/oplata-italian');
    }
  }, [navigate]);
  ...
  const handleBuyClick = async (e) => {
    ...
    if (isValid) {
      try {
        ...
        // Создаем платеж
        const language = localStorage.getItem('courseLanguage') || 'french';
...
export default OplataFrenchPage;